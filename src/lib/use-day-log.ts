...
function loadInitial(): MealRecord[] {
  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed.date === today && Array.isArray(parsed.meals)) {
        loadedStoredDay = true;
        return parsed.meals;
      }
    }
  } catch {
...
export function duplicateMeal(mealId: string) {
  ...
  const duplicated: MealRecord = {
    ...
    name: `${meal.name} copy`,
    loggedAt: new Date().toISOString(),
    items: meal.items.map((item) => ({ ...item, id: nextId("item") })),
  };
  persist([...meals, duplicated]);
  return duplicated;
}
