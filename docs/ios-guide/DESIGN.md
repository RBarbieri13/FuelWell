...
>
> **Reconciliation TODO:** brand green is `#3D9B2F` in `src/lib/design-tokens.ts` but commit `63419cb` updated the logo green to `#47e7b0`. Robert + Max to confirm which is canonical before iOS UI work begins.
...
Color tokens are dark-mode native. Light mode is deferred until post-Pilot.

### Primary

| Token | Hex | Use |
|---|---|---|
| `color.primary.orange` | `#E87A1D` | Primary brand accent, CTAs in marketing context |
| `color.primary.green` | `#3D9B2F` | Primary action, success-leaning |
| `color.primary.greenLogo` | `#47E7B0` | Logo-only green (per commit `63419cb`) — **reconcile** |
| `color.primary.accent` | `#00D278` | Highlight, success state, decision-engine verdict positive |

### Background

| Token | Hex | Use |
|---|---|---|
| `color.bg.base` | `#0A0A0F` | Root window |
| `color.bg.surface` | `#111118` | Cards, sheets |
| `color.bg.elevated` | `#1A1A24` | Modals, popovers, raised cards |
| `color.bg.border` | `#2A2A3A` | Dividers, card borders |

### Text

| Token | Hex | Use |
|---|---|---|
| `color.text.primary` | `#FFFFFF` | Headlines, verdicts |
| `color.text.body` | `#E0E0E0` | Body copy |
| `color.text.secondary` | `#999999` | Labels, captions |
| `color.text.muted` | `#777777` | Placeholder, helper text |
...
| `gradient.header` | `linear-gradient(135deg, #1A1A2E, #16213E)` | Top-of-screen washes |
| `gradient.premiumGlow` | `linear-gradient(135deg, #2A1A3E, #1A2A3E)` | Premium-feature backgrounds |
| `gradient.text` | `linear-gradient(90deg, #00D278, #00B4D8)` | Display-type emphasis |
...
- Section labels: `font.body` uppercase + `text.caption`, `color.text.secondary`
...
Shadows are subtle on dark backgrounds. Elevation is primarily expressed by background lightening (`bg.surface` → `bg.elevated`).

| Token | Definition |
|---|---|
| `elevation.0` | none |
| `elevation.1` | `0 1px 2px rgba(0,0,0,0.3)` — base cards |
| `elevation.2` | `0 4px 12px rgba(0,0,0,0.4)` — raised cards |
| `elevation.3` | `0 12px 32px rgba(0,0,0,0.5)` — modals, popovers |
...
System: SF Symbols (iOS native). Custom icons only when SF Symbols can't express the concept (rare). Custom icons live in `Packages/DesignSystem/Resources/Icons/` as PDF vectors with a single fill color, tinted via `Theme`.
...
- [ ] Reconcile primary green: `#3D9B2F` (tokens) vs `#47E7B0` (logo). Decision in `decisions.md`.
- [ ] Light mode timing — Pilot is dark-only. When does light mode enter the plan?
- [ ] Custom icon set — needed for 7 features, or stay SF-Symbols-only at Pilot?
- [ ] Haptics — this document doesn't yet specify a haptic vocabulary. Add before Phase 3.
