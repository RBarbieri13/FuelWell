#!/usr/bin/env node
/**
 * Build the LITE Phase 0.5 Review PDF (~4-5 MB).
 * Pipeline:
 *  1) For each <slug>.png in docs/ios-guide/mockups/, downsample with sharp to
 *     900-wide JPEG quality 72 mozjpeg in /tmp/fw-mockups-lite/<slug>.jpg.
 *  2) Reuse the same combined-PDF HTML but with .jpg paths.
 *  3) Add a LITE badge to the cover.
 *
 * Output: docs/ios-guide/pdfs/FuelWell-Phase-0.5-Review-LITE.pdf
 */
const fs = require('fs');
...
const LITE_DIR = '/tmp/fw-mockups-lite';
const OUT = path.join(REPO, 'docs/ios-guide/pdfs/FuelWell-Phase-0.5-Review-LITE.pdf');
...
// Re-import SCREENS from the combined builder by requiring its module-style export.
// Cheap approach: parse the file. Instead, redefine the same catalog here.
// To stay in sync, both files share this constant — keep them in lockstep when editing.
const { SCREENS } = require('./screens-catalog.js');

function tagBadge(tag) {
  if (!tag) return '';
  const colors = { NEW: '#00D278', REDESIGN: '#A855F7', UPDATED: '#00B4D8', REFERENCE: '#9CA3AF', LEGACY: '#EF4444' };
  return `<span class="tag" style="background:${colors[tag] || '#6B7280'}">${tag}</span>`;
}
...
async function compressAll() {
  if (!fs.existsSync(LITE_DIR)) fs.mkdirSync(LITE_DIR, { recursive: true });
  const slugs = SCREENS.map(s => s.slug);
  let totalBefore = 0, totalAfter = 0;
  for (const slug of slugs) {
    const src = path.join(PNG_DIR, slug + '.png');
    const dst = path.join(LITE_DIR, slug + '.jpg');
    if (!fs.existsSync(src)) { console.warn(`  [skip] missing ${slug}.png`); continue; }
    ...
    console.log(`  [lite] ${slug}: ${(before/1024).toFixed(0)}KB -> ${(after/1024).toFixed(0)}KB`);
  }
  console.log(`[compress] total ${(totalBefore/1024/1024).toFixed(1)}MB -> ${(totalAfter/1024/1024).toFixed(1)}MB`);
}
...
function buildHtml() {
  ...
  for (const s of SCREENS) {
    ...
    const jpgPath = path.join(LITE_DIR, s.slug + '.jpg');
    const exists = fs.existsSync(jpgPath);
    deckHtml += `<div class="mockup-page">
      <div class="mockup-header">
        <div class="mockup-section">${s.section}</div>
        <h2 class="mockup-title">${s.title} ${tagBadge(s.tag)}</h2>
        <div class="mockup-slug">${s.slug}.jpg</div>
      </div>
      <div class="mockup-image-wrap">
        ${exists ? `<img src="file://${jpgPath}" />` : `<div class="missing">missing JPG: ${s.slug}</div>`}
      </div>
    </div>`;
  }
    ...
    ? `
      ...
      (async () => {
        for (const b of blocks) {
          ...
          try { const { svg } = await mermaid.render(b.id + '-svg', b.code); host.innerHTML = svg; }
          catch (e) { host.innerHTML = '<pre style="color:#EF4444">mermaid render failed: ' + e.message + '</pre>'; }
        }
        window.__mermaid_done__ = true;
      })();`
    : `window.__mermaid_done__ = true;`;

  // Same styles as combined; LITE badge added on cover.
  return `<!DOCTYPE html><html><head><meta charset="utf-8">
<title>FuelWell · Phase 0.5 Review · LITE</title>
...
<style>
  :root { --bg:#F4F5F7; --surface:#FFFFFF; --ink:#0A0A0F; --body:#1F2937; --muted:#6B7280; --border:#E5E7EB; --soft:#EEF0F3; --action:#00D278; --inverted:#0F1117; }
  * { box-sizing: border-box; }
  ...
  .cover { padding:100px 56px 64px; background:linear-gradient(135deg,#F4F5F7,#FFFFFF); border-bottom:6px solid var(--action); min-height:95vh; display:flex; flex-direction:column; justify-content:space-between; }
  ...
  .lite-badge { display:inline-block; padding:6px 14px; background:#F59E0B; color:#fff; font-family:'DM Sans',sans-serif; font-weight:700; font-size:10pt; letter-spacing:0.12em; border-radius:6px; margin-bottom:18px; text-transform:uppercase; }
  ...
  .missing { color:#EF4444; padding:24px; border:2px dashed #EF4444; border-radius:8px; font-family:'JetBrains Mono',ui-monospace,monospace; }
...
<section class="cover">
  <div>
    ...
    <span class="lite-badge">LITE · compressed for email</span>
    <h1>Phase 0.5<br/>Review Packet</h1>
    <div class="lede">Same content as the full PDF, with mockup images downsampled to 900px JPEG q72. Use the full version for high-resolution review; this LITE build is sized for inbox-friendly distribution.</div>
    <div class="stats">
      <div class="stat"><div class="num">${SCREENS.filter(s=>s.tag!=='LEGACY').length}</div><div class="lbl">Mockups</div></div>
      <div class="stat"><div class="num">${tocSections.length}</div><div class="lbl">Sections</div></div>
      <div class="stat"><div class="num">5</div><div class="lbl">Tabs</div></div>
    </div>
  </div>
...
<section class="toc"><h2>Contents</h2>
  ${tocSections.map(sec => `<div class="toc-section"><div class="name">${sec}</div><div class="count">${SCREENS.filter(s => s.section === sec).length} screens</div></div>`).join('')}
</section>
<section class="doc-cover"><div class="kicker">Spec doc</div><h1>App Map v2.1</h1><div class="desc">Tab structure, screen inventory, navigation rules.</div></section>
<section class="doc">${appMap.html}</section>
<section class="doc-cover"><div class="kicker">Spec doc</div><h1>Flow Chart v2.1</h1><div class="desc">Per-screen flows, cross-cutting rules.</div></section>
...
async function main() {
  console.log('[build-compressed] step 1: downsample PNGs');
  await compressAll();

  console.log('[build-compressed] step 2: assemble LITE PDF');
  ...
  await page.pdf({
    ...
    footerTemplate: `<div style="font-family:Inter,sans-serif;font-size:8pt;color:#6B7280;width:100%;padding:0 12mm;display:flex;justify-content:space-between;"><span>FuelWell · Phase 0.5 Review · LITE · ${new Date().toISOString().slice(0,10)}</span><span class="pageNumber"></span></div>`,
    margin: { top: '14mm', bottom: '16mm', left: '12mm', right: '12mm' },
  });
  await browser.close();
  fs.unlinkSync(tmpHtml);
  const sz = (fs.statSync(OUT).size / (1024 * 1024)).toFixed(2);
  console.log(`[build-compressed] wrote ${OUT} (${sz} MB)`);
}
